...
# Medicare Advantage overrides: same insurer name, different governing answer.
# A UHC MA member was silently getting the commercial InterQual answer; these
# rows (data/policy_platform/ma_plan_overrides.json, each citing the payer's
# own published MA document) let the app answer by plan kind.
ma = {}
_ma_path = ROOT/'data/policy_platform/ma_plan_overrides.json'
if _ma_path.exists():
    for ov in json.loads(_ma_path.read_text())['overrides']:
        for cpt in ov['cpts']:
            ma[f"{ov['insurer']}|{cpt}"] = [ov['code'], uix(ov['policy_url']),
                                            ov['policy_title'][:60], ov['why']]

url_list = [u for u,_ in sorted(urls.items(), key=lambda x: x[1])]
payload = {'urls': url_list, 'comm': comm, 'medicare': medicare, 'medicaid': medicaid, 'ma': ma}
